...
def test_ansible_restore_only_uses_checkpoint_path(tmp_path: Path) -> None:
    ...
    assert "ansible_collections/tomazb/acm_switchover/playbooks/restore_only.yml" in command
...
def test_build_extra_vars_includes_all_operation_defaults(tmp_path: Path) -> None:
    adapter = _make_adapter(tmp_path)

    extra_vars = adapter.build_extra_vars("ansible-passive-switchover")

    op = extra_vars["acm_switchover_operation"]
    assert "method" in op
    assert "old_hub_action" in op
    assert "activation_method" in op
    assert "min_managed_clusters" in op
    features = extra_vars["acm_switchover_features"]
    assert "token_expiry_warning_hours" in features
    assert "skip_observability_checks" in features


def test_build_extra_vars_sets_method_full_for_restore_only(tmp_path: Path) -> None:
    adapter = _make_adapter(tmp_path)
...
def test_build_env_sets_ansible_collections_path(tmp_path: Path) -> None:
    adapter = _make_adapter(tmp_path)

    env = adapter._build_env()

    assert "ANSIBLE_COLLECTIONS_PATH" in env
    assert str(Path("/repo")) in env["ANSIBLE_COLLECTIONS_PATH"]
...
def test_ansible_adapter_execute_handles_timeout(monkeypatch, tmp_path: Path) -> None:
    def fake_run(command, cwd, text, capture_output, check, timeout, env):
    ...
    assert Path(result.stdout_path).read_text(encoding="utf-8") == "partial output"
    assert Path(result.stderr_path).read_text(encoding="utf-8") == "partial error"


def test_ansible_adapter_execute_surfaces_redaction_rejection(monkeypatch, tmp_path: Path) -> None:
