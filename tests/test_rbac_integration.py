...
class TestRBACPermissionCoverage:
    ...
    def test_namespace_permissions_include_backupstoragelocations(
        ...
        assert (
            len(bsl_perm) == 1
        ), "Expected backupstoragelocations permission in backup namespace"
        assert (
            bsl_perm[0][0] == "velero.io"
        ), "BackupStorageLocations should use velero.io API group"
        assert "get" in bsl_perm[0][2], "Expected 'get' verb for backupstoragelocations"
        assert (
            "list" in bsl_perm[0][2]
        ), "Expected 'list' verb for backupstoragelocations"
    ...
    def test_namespace_permissions_include_secrets_for_observability(
        ...
        assert (
            len(secrets_perm) == 1
        ), "Expected secrets permission in observability namespace"
    ...
    def test_namespace_permissions_include_routes_for_observability(
        ...
        assert (
            len(routes_perm) == 1
        ), "Expected routes permission in observability namespace"
        assert (
            routes_perm[0][0] == "route.openshift.io"
        ), "Routes should use route.openshift.io API group"
    ...
    def test_namespace_permissions_include_agent_namespace(self, validator_permissions):
        ...
        assert (
            ...
            in validator_permissions["managed_cluster_namespace"]
        ), "Expected open-cluster-management-agent namespace in MANAGED_CLUSTER_NAMESPACE_PERMISSIONS"

        agent_perms = validator_permissions["managed_cluster_namespace"][
            "open-cluster-management-agent"
        ]
        ...
        assert (
            len(deployments_perm) == 1
        ), "Expected deployments permission in agent namespace"
        assert deployments_perm[0][0] == "apps", "Deployments should use apps API group"
        assert (
            "patch" in deployments_perm[0][2]
        ), "Expected 'patch' verb for deployments"

    def test_backup_namespace_includes_delete_for_backupschedules(
        self, validator_permissions
    ):
        ...
        assert (
            len(bs_perm) == 1
        ), "Expected backupschedules permission in backup namespace"
        assert (
            "delete" in bs_perm[0][2]
        ), "Expected 'delete' verb for backupschedules (used in primary_prep.py)"
    ...
    def test_cluster_permissions_include_managed_clusters(self, validator_permissions):
        ...
        mc_perm = [
            p for p in validator_permissions["cluster"] if p[1] == "managedclusters"
        ]
    ...
    def test_cluster_permissions_include_nodes(self, validator_permissions):
        ...
        assert (
            len(nodes_perm) == 1
        ), "Expected nodes permission for cluster health check"
    ...
    def test_cluster_permissions_include_clusteroperators(self, validator_permissions):
        ...
        co_perm = [
            p for p in validator_permissions["cluster"] if p[1] == "clusteroperators"
        ]

        assert (
            len(co_perm) == 1
        ), "Expected clusteroperators permission for OpenShift health check"
        assert (
            co_perm[0][0] == "config.openshift.io"
        ), "ClusterOperators should use config.openshift.io API group"
    ...
    def test_cluster_permissions_include_clusterversions(self, validator_permissions):
        ...
        cv_perm = [
            p for p in validator_permissions["cluster"] if p[1] == "clusterversions"
        ]

        assert (
            len(cv_perm) == 1
        ), "Expected clusterversions permission for upgrade status check"
        assert (
            cv_perm[0][0] == "config.openshift.io"
        ), "ClusterVersions should use config.openshift.io API group"
...
class TestRBACManifestConsistency:
    ...
    def test_kustomize_clusterrole_has_argocd_rules(
        self, kustomize_clusterrole_content
    ):
        ...
        for snippet in expected_snippets:
            assert (
                snippet in kustomize_clusterrole_content
            ), f"Missing Argo CD snippet in static clusterrole: {snippet}"
    ...
    def test_helm_clusterrole_has_argocd_rules(self, helm_clusterrole_content):
        ...
        for snippet in expected_snippets:
            assert (
                snippet in helm_clusterrole_content
            ), f"Missing Argo CD snippet in Helm clusterrole: {snippet}"
