...
class E2ETestAnalyzer:
        ...
        try:
            if PANDAS_AVAILABLE:
                ...
                for _, row in df.iterrows():
                    cycle = (
                        int(row["cycle"].replace("cycle_", "")) if isinstance(row["cycle"], str) else int(row["cycle"])
                    )
    ...
    def analyze_performance(self) -> Dict[str, Any]:
        ...
        performance = {"cycle_performance": {}, "phase_performance": {}, "overall_metrics": {}}

        # Analyze each cycle
        for cycle, phases in self.cycles_data.items():
            cycle_perf = {"total_duration": 0, "successful_phases": 0, "failed_phases": 0, "phases": phases}
        ...
        for cycle, phases in self.cycles_data.items():
            for phase, data in phases.items():
                if phase not in phase_stats:
                    phase_stats[phase] = {"durations": [], "success_count": 0, "failure_count": 0}
        ...
        successful_cycles = sum(
            1 for cycle, perf in performance["cycle_performance"].items() if perf["failed_phases"] == 0
        )
        ...
        performance["overall_metrics"] = {
            ...
            "avg_cycle_duration": (
                sum(perf["total_duration"] for perf in performance["cycle_performance"].values()) / total_cycles
                if total_cycles > 0
                else 0
            ),
        }
            ...
            if timestamp:
                try:
                    ...
                    alert_analysis["timeline"].append(
                        {"timestamp": dt, "type": alert_type, "hub": hub_type, "message": alert.get("message", "")}
                    )
                except ValueError:
                    pass
    ...
    def analyze_resource_trends(self) -> Dict[str, Any]:
        ...
        for metric in self.metrics_data:
            ...
            trends["timeline"].append({"timestamp": timestamp, "iso_timestamp": iso_timestamp})
...
"""
        ...
        for phase, stats in performance["phase_performance"].items():
            success_rate_class = (
                "success" if stats["success_rate"] >= 90 else "warning" if stats["success_rate"] >= 70 else "error"
            )
...
"""
    ...
    def generate_comparison_report(
        self, baseline_dir: str, output_file: str = "./e2e_comparison_report.html", regression_threshold: float = 1.2
    ) -> bool:
    ...
    def _compare_runs(self, current: Dict, baseline: Dict, threshold: float) -> Dict[str, Any]:
        ...
        comparison = {"overall": {}, "phases": {}, "regressions": [], "improvements": []}
        ...
        comparison["overall"] = {
            ...
            "duration_ratio": (
                current_overall["avg_cycle_duration"] / baseline_overall["avg_cycle_duration"]
                if baseline_overall["avg_cycle_duration"] > 0
                else 1.0
            ),
        }
            ...
            comparison["regressions"].append(
                {
                    ...
                    "ratio": (
                        current_overall["overall_success_rate"] / baseline_overall["overall_success_rate"]
                        if baseline_overall["overall_success_rate"] > 0
                        else 0
                    ),
...
"""
        ...
        duration_class = (
            "error" if overall["duration_ratio"] > threshold else "success" if overall["duration_ratio"] < 1 else ""
        )
        sr_class = (
            "error" if overall["success_rate_delta"] < -5 else "success" if overall["success_rate_delta"] > 5 else ""
        )
...
"""
        ...
        for phase, data in comparison["phases"].items():
            ...
            ratio_class = (
                "delta-positive" if p95_ratio > threshold else "delta-negative" if p95_ratio < 1 / threshold else ""
            )
...
def main():
    ...
    parser.add_argument("--compare", metavar="BASELINE_DIR", help="Compare against a baseline results directory")
    parser.add_argument(
    ...
    parser.add_argument(
        "--check-regressions", action="store_true", help="Exit with code 1 if regressions are detected (for CI)"
    )
