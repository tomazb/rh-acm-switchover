...
DELETE_MANAGED_CLUSTERS = (
    ROLES_DIR / "decommission" / "tasks" / "delete_managed_clusters.yml"
)
...
class TestDecommissionMain:
    ...
    def test_confirmation_gate_exists(self):
        ...
        confirmation_gate = [
            t
            for t in fail_tasks
            if "confirmed" in str(t.get("ansible.builtin.fail", {}).get("msg", ""))
            or "confirmed" in _when_text(t)
        ]
    ...
    def test_confirmation_gate_bypassed_only_in_dry_run(self):
        ...
        assert (
            confirmation_gate
        ), "Confirmation gate must exist (see test_confirmation_gate_exists)"
    ...
    def test_primary_hub_assertion_exists(self):
        ...
        hub_asserts = [
            t
            for t in assert_tasks
            if "primary" in str(t.get("ansible.builtin.assert", {}).get("that", ""))
        ]
        assert hub_asserts, (
    ...
    def test_primary_hub_assertion_checks_kubeconfig_and_context(self):
        ...
        for task in hub_asserts:
            ...
            assert isinstance(
                conditions, list
            ), "Assert 'that' must be a list of conditions"
            conditions_text = " ".join(str(c) for c in conditions)
            assert (
                "kubeconfig" in conditions_text
            ), "Primary hub assert must check kubeconfig is non-empty"
            assert (
                "context" in conditions_text
            ), "Primary hub assert must check context is non-empty"
    ...
    def test_rbac_validated_before_destructive_operations(self):
        ...
        assert (
            "validate_rbac.yml" in filenames
        ), "decommission/tasks/main.yml must include validate_rbac.yml"
    ...
    def test_observability_deletion_is_conditional(self):
        ...
        include_tasks = [t for t in self.tasks if "ansible.builtin.include_tasks" in t]
        obs_includes = [
            t for t in include_tasks if _include_file(t) == "delete_observability.yml"
        ]
        assert (
            obs_includes
        ), "decommission/tasks/main.yml must include delete_observability.yml"
...
class TestDeleteObservability:
    ...
    def test_file_exists(self):
        assert (
            DELETE_OBSERVABILITY.exists()
        ), "decommission/tasks/delete_observability.yml must exist"
    ...
    def test_dry_run_announces_without_acting(self):
        ...
        dry_run_announce = [
            ...
            if "dry_run" in _when_text(t) and "==" in _when_text(t)
        ]
    ...
    def test_list_task_skipped_in_dry_run(self):
        ...
        mco_list_tasks = [
            ...
            if t.get("kubernetes.core.k8s_info", {}).get("kind")
            == "MultiClusterObservability"
        ]
        assert (
            mco_list_tasks
        ), "delete_observability.yml must list MultiClusterObservability resources"
    ...
    def test_delete_task_guarded_by_execute_mode(self):
        ...
        delete_tasks = [
            t
            for t in k8s_tasks
            if t.get("kubernetes.core.k8s", {}).get("state") == "absent"
        ]
        assert (
            delete_tasks
        ), "delete_observability.yml must have a delete task (state: absent)"
    ...
    def test_delete_task_uses_loop(self):
        ...
        for task in delete_tasks:
            assert (
                "loop" in task
            ), "Delete task must use 'loop' to handle all discovered MCO instances"
    ...
    def test_wait_task_has_retries_delay_until(self):
        ...
        wait_tasks = [
            t
            for t in k8s_info_tasks
            if t.get("kubernetes.core.k8s_info", {}).get("kind") == "Pod"
        ]
    ...
    def test_wait_task_handles_notfound_gracefully(self):
        ...
        for task in wait_tasks:
            ...
            failed_when_text = (
                " ".join(str(c) for c in failed_when)
                if isinstance(failed_when, list)
                else str(failed_when)
            )
            assert (
                "NotFound" in failed_when_text
                or "not found" in failed_when_text.lower()
            ), (
...
class TestDeleteManagedClusters:
    ...
    def test_file_exists(self):
        assert (
            DELETE_MANAGED_CLUSTERS.exists()
        ), "decommission/tasks/delete_managed_clusters.yml must exist"
    ...
    def test_list_task_skipped_in_dry_run(self):
        ...
        assert (
            mc_list_tasks
        ), "delete_managed_clusters.yml must list ManagedCluster resources"
        list_task = mc_list_tasks[0]
        assert "!= 'dry_run'" in _when_text(
            list_task
        ), "ManagedCluster list must be guarded by execute-mode check"
    ...
    def test_local_cluster_excluded_from_deletion_targets(self):
        ...
        target_selection = [
            ...
            if "_managed_cluster_delete_targets"
        ...
        for task in target_selection:
            task_text = str(task)
            assert (
                "local-cluster" in task_text
            ), "Deletion target selection must explicitly exclude 'local-cluster'"
            assert (
...
class TestDeleteMultiClusterHub:
    ...
    def test_file_exists(self):
        assert (
            DELETE_MCH.exists()
        ), "decommission/tasks/delete_multiclusterhub.yml must exist"
    ...
    def test_list_and_delete_guarded_by_execute_mode(self):
        ...
        assert (
            mch_list_tasks
        ), "delete_multiclusterhub.yml must list MultiClusterHub resources"
        for task in mch_list_tasks:
            assert "!= 'dry_run'" in _when_text(
                task
            ), "MCH list must be guarded by execute-mode check"
        ...
        mch_delete_tasks = [
            t
            for t in k8s_tasks
            if t.get("kubernetes.core.k8s", {}).get("kind") == "MultiClusterHub"
        ]
        assert (
            mch_delete_tasks
        ), "delete_multiclusterhub.yml must have a MultiClusterHub delete task"
        for task in mch_delete_tasks:
            assert "!= 'dry_run'" in _when_text(
                task
            ), "MCH delete must be guarded by execute-mode check"
