...
def _assert_python_identity_binder_contract(source: str) -> None:
    ...
    function = next(
        ...
        and node.name == "_bind_runtime_hub_identities"
    )
    ...
    collect_index = next(
        ...
        and _call_name(call) == "_collect_hub_identities"
    )
    ...
    ensure_index = next(
        ...
        for index, statement in enumerate(function.body)
        if (call := _direct_call(statement)) is not None
        and _call_name(call) == "state.ensure_hub_identities"
    )
        ...
        for node in ast.walk(tree):
            ...
            for imported_name in imported_names:
                if path.is_relative_to(COLLECTION_ROOT):
                    ...
                    prohibited = (
                        imported_name == "ansible_collections"
                ...
                if (
                    prohibited
                    or imported_name == "tests.release"
                    or imported_name.startswith("tests.release.")
                ):
                    violations.append(
                        f"{path.relative_to(REPOSITORY_ROOT)}: {imported_name}"
                    )
...
def test_python_binds_fresh_identities_before_cross_role_validation_and_state() -> None:
    ...
    input_source = _function_source(
        REPOSITORY_ROOT / "lib" / "validation.py", "validate_all_cli_args"
    )
...
def test_python_identity_binder_contract_rejects_synthetic_source_mutations() -> None:
    ...
    mutated_source = source.replace(
        "validate_distinct_hub_identities(hub_identities)", "pass", 1
    )
...
def test_collection_preflight_keeps_checkpoint_control_outside_identity_evidence() -> (
    None
):
...
def test_collection_static_boundary_excludes_obsolete_discovery_and_single_hub_workflows() -> (
    None
):
    """Only normal flows use the cross-role barrier and post-barrier recovery boundary."""
    playbooks = COLLECTION_ROOT / "playbooks"
    switchover = yaml.safe_load(
        (playbooks / "switchover.yml").read_text(encoding="utf-8")
    )
    outer = next(
        task
        for task in switchover[0]["tasks"]
        if task["name"] == "Run switchover phases with reporting"
    )
    barrier_index = next(
        index
    ...
    recovery_index = next(
        index
        for index, task in enumerate(outer["block"])
        if task["name"] == "Run post-barrier switchover phases"
    )
    ...
    restore = yaml.safe_load(
        (playbooks / "restore_only.yml").read_text(encoding="utf-8")
    )
    restore_roles = [
...
def test_preflight_rbac_validates_hubs_through_shared_hub_loop():
    ...
    hub_includes = [
        t
        for t in tasks
        if t.get("ansible.builtin.include_tasks") == "validate_rbac_hub.yml"
    ]
    assert (
        len(hub_includes) == 1
    ), "validate_rbac.yml must drive hub validation through exactly one shared include"
    ...
    table_task = next(
        t
        for t in tasks
        if "_rbac_hub_validations" in t.get("ansible.builtin.set_fact", {})
    )
    ...
    assert "restore_only" in str(
        primary["enabled"]
    ), "primary hub entry must be disabled in restore-only mode"
    assert (
        secondary["enabled"] is True
    ), "secondary hub validation must be unconditional"
...
def test_preflight_rbac_hub_loop_preserves_registered_fact_names():
    ...
    for published in [
        ...
        assert (
            published in hub_text
        ), f"validate_rbac_hub.yml must re-publish {published}"
...
def test_preflight_rbac_hub_loop_keeps_primary_only_and_secondary_only_behavior():
    ...
    assert (
        ...
        in primary["include_old_hub_finalization"]
    )
    ...
    assert "restore_only" in str(
        old_hub_task.get("when", "")
    ), "old-hub finalization must stay primary-only input"
...
def test_preflight_rbac_managed_cluster_validation_stays_separate_from_hub_loop():
    ...
    includes = [
        ...
        for t in tasks
        if t.get("ansible.builtin.include_tasks")
    ]
