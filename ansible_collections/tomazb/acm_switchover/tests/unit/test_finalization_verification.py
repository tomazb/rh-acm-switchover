...
def test_main_cleans_restores_before_enabling_backups():
    ...
    assert includes.index("cleanup_restores.yml") < includes.index("enable_backups.yml"), (
...
def test_verify_backups_waits_for_clean_completed_acm_owned_velero_backup():
    ...
    timeout_expr = str(
        timeout_task["ansible.builtin.set_fact"]["_acm_secondary_backup_verify_timeout_seconds"]
    )
    assert "+ 600" in timeout_expr, (
    ...
    assert "PartiallyFailed" in terminal_until and "FailedValidation" in terminal_until, (
    ...
    assert "PartiallyFailed" in text and "FailedValidation" in text, (
    ...
    assert "sort(attribute='metadata.creationTimestamp')" in text, (
...
def test_verify_mch_requires_running_phase_and_healthy_pods():
    ...
    assert "Running" in str(mch_wait_task.get("until", "")), (
    ...
    assert "Running" in until and "Succeeded" in until, (
