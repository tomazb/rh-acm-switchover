...
| Decommission old hub | dual-supported | `playbooks/decommission.yml` | Requires explicit non-empty primary kubeconfig/context plus `acm_switchover_decommission.confirmed: true` or `mode: dry_run`; MultiClusterHub teardown records the target UID and the captured operator identity before a UID-guarded delete, then classifies every remaining ACM Pod by owner chain up to the recorded operator Deployment, and fails closed on drain timeout, on an unreadable pass, or on a recorded identity that no longer holds — the same contract as the Python CLI (#290). Its RBAC surface now also requires the `open-cluster-management` reads `clusterserviceversions` (get, list), `deployments` (get) and `replicasets` (get) plus `multiclusterhubs` get on the decommission extension; see `docs/deployment/rbac-requirements.md`. MultiClusterObservability deletion goes through a UID-preconditioned guarded delete with a durable per-resource phase record and a fresh completion proof; the standalone playbook declares `acm_switchover_standalone_decommission: true`, never evaluates the destination-observability gate, and refuses a truthy `acknowledge_observability_not_migrated` in `pre_tasks` before any read |
