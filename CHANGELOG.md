...
- Python and collection post-activation klusterlet remediation now re-checks wrong-hub clusters after remediation and fails when remediation failed or klusterlets still point at the wrong hub, while keeping missing managed-cluster kubeconfigs as explicit skips unless ManagedCluster readiness is unhealthy.
