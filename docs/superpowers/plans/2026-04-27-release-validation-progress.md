...
| 06 | `docs/superpowers/plans/2026-04-27-release-validation-06-ansible-stream-adapter-schema-stability.md` | Plans 01-05 | `verified` | session `9b890195-2e7f-4eb2-a5d4-30796ca0f3be` | `ansible` (commits `4b86d67`–`cd1735f`) | `python -m pytest tests/release/adapters/test_ansible.py ansible_collections/tomazb/acm_switchover/tests/unit/test_release_source_schema.py -q` | 14 passed, 3 skipped (ansible-core absent in venv). Post-review fix commit `cd1735f`: adapter now emits `acm_switchover_execution` nested dict (report_dir/checkpoint.path); `acm_switchover_features.argocd.manage: true` for argocd-managed-switchover; ValueError for unknown scenario; timeout=3600 + JSON/OSError guard in discover_reports; schema stability tests rewritten to call real module functions with pytest.importorskip. |
