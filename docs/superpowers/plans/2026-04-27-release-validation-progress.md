...
| 06 | `docs/superpowers/plans/2026-04-27-release-validation-06-ansible-stream-adapter-schema-stability.md` | Plans 01-05 | `verified` | session `9b890195-2e7f-4eb2-a5d4-30796ca0f3be` | `ansible` (commits `4b86d67`–`5439e71`) | `python -m pytest tests/release/adapters/test_ansible.py ansible_collections/tomazb/acm_switchover/tests/unit/test_release_source_schema.py -q` | 11 passed. AnsibleAdapter with build_command, execute (cwd=collection_root), discover_reports; ANSIBLE_SCENARIOS set and execute_ansible_scenarios wired; 3 collection schema stability tests pinning preflight/switchover/checkpoint fields. |
