# Primary-Prep Mutation Baseline Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Run a whole-file mutation-testing baseline for `modules/primary_prep.py`, using the Python primary-prep tests plus focused collection unit and integration lanes to classify parity-sensitive survivors before any fix work starts.

**Architecture:** This is a baseline/triage plan, not a survivor-kill plan. The workflow first proves the unmutated Python and collection primary-prep lanes are green, then temporarily repoints `[mutmut]` in `setup.cfg` to `modules/primary_prep.py`, runs the spike, classifies the top survivors against both form factors, records the baseline, and restores the default `setup.cfg` target afterward.

**Tech Stack:** Python 3.10+, pytest, mutmut 3.6.x, Python CLI primary-prep workflow, Ansible collection primary-prep role/unit tests
...
**Files:**
- Reference: `docs/plans/2026-06-10-primary-prep-mutation-baseline-design.md`
...
## Task 2: Prove the Python primary-prep baseline is green

**Files:**
- Reference: `modules/primary_prep.py`
- Test: `tests/test_primary_prep.py`
...
## Task 3: Prove the collection primary-prep unit/contracts lane is green

**Files:**
- Test: `ansible_collections/tomazb/acm_switchover/tests/unit/test_primary_prep_auto_import.py`
- Test: `ansible_collections/tomazb/acm_switchover/tests/unit/test_backup_schedule_persistence.py`
...
PYTHONPATH=. python -m pytest \
  ...
  ansible_collections/tomazb/acm_switchover/tests/unit/test_backup_schedule_persistence.py \
  -q
```
...
## Task 4: Prove the collection primary-prep integration/scenario lane is green
...
## Task 5: Temporarily point mutmut at `modules/primary_prep.py`

**Files:**
- Modify: `setup.cfg`

**Step 1: Update the `[mutmut]` block for the primary-prep spike**

Change `setup.cfg` so the block reads:

```ini
[mutmut]
source_paths = modules/primary_prep.py
...
Expected: `source_paths` points at `modules/primary_prep.py`, the Python primary-prep test file is selected, `also_copy` includes both `lib/` and `modules/`, and only the narrow logger/raise patterns are configured.

**Step 3: Commit the temporary spike config**

```bash
git add setup.cfg
git commit -m "chore: point mutmut at modules/primary_prep.py baseline spike"
```

## Task 6: Run the whole-file primary-prep mutation spike

**Files:**
- Source: `modules/primary_prep.py`
- Baseline output: `mutants/modules/primary_prep.py.meta`
...
Expected: the run completes and writes `mutants/modules/primary_prep.py.meta`.
...
meta = json.load(open('mutants/modules/primary_prep.py.meta'))
...
other = Counter(v for v in exit_codes.values() if v not in (0, 1, None))
print(f'Total: {len(exit_codes)} | Killed: {killed} | Survived: {survived} | Not checked: {not_checked}')
print('Other statuses:', dict(other))
...
Expected: a clear total/killed/survived/not_checked summary, any nonstandard status buckets, and the top survivor-heavy functions.
...
- Source: `modules/primary_prep.py`
- Test: `tests/test_primary_prep.py`
- Reference: collection primary-prep test files from Tasks 3-4
...
- BackupSchedule pause/delete semantics and wrong-resource targeting
- disable-auto-import targeting and local-cluster exclusion
- observability / Thanos scale-down and wait semantics
- Argo CD prep coordination when enabled
...
Add a new Phase 2 baseline subsection for `modules/primary_prep.py` with:
...
- any additional status buckets if present
...
git commit -m "docs: record modules/primary_prep.py mutation baseline"
```
...
```bash
git add setup.cfg
git commit -m "chore: restore mutmut config after primary-prep baseline spike"
```
