...
`delete` on `multiclusterobservabilities`: a standalone decommission run does not carry
the baseline operator ClusterRole, and the observability teardown reads the named
MultiClusterObservability before deleting it and again for its final absence proof.
