...
| `lib/strict_read.py` + `lib/kube_client.py` strict producers (`list_custom_resources_strict`, `get_custom_resource_strict`, `get_namespace_strict`, `list_pods_strict`, `get_deployment_strict`, `get_replicaset_strict`) | `plugins/module_utils/k8s_read.py`, with `plugins/modules/acm_k8s_read_outcome.py` as the thin module wrapper over those helpers. A named 404 is `not_found` only after live discovery confirms the route that was read; a built-in 404 that live discovery does not confirm is `error` or `kind_not_served` where Python reports absence (approved divergence, #317, see [coexistence](../../ansible_collections/tomazb/acm_switchover/docs/coexistence.md)) | 3 |
