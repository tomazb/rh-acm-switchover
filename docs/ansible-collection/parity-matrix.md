...
| activation | dual-supported | dual-supported | core parity requirement. Approved fail-closed divergence (#317): a named built-in read (the `import-controller-config` ConfigMap) whose 404 cannot be confirmed by readable live discovery is an error in the collection and absence in Python — see [coexistence](../../ansible_collections/tomazb/acm_switchover/docs/coexistence.md) |
