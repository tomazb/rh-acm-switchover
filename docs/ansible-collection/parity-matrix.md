...
| activation | dual-supported | dual-supported | core parity requirement. Approved fail-closed divergence (#317): a named built-in read (the `import-controller-config` ConfigMap) whose named 404 live discovery does not confirm is `error` (discovery unreadable, or a route mismatch) or `kind_not_served` (kind omitted) in the collection and absence in Python — see [coexistence](../../ansible_collections/tomazb/acm_switchover/docs/coexistence.md) |
