...
| decommission | dual-supported | dual-supported | Collection defaults observability detection to `auto`, waits for ACM/observability workload pods during teardown, and warns if ACM workload pods remain after the bounded MultiClusterHub deletion wait. Both form factors now carry durable teardown state before the first delete and fail closed without it: Python through `RunRecord`, the Collection through a `decommission` checkpoint phase owned by `playbooks/decommission.yml`. The durable formats stay independent and non-interchangeable — what is held in parity is the operator-visible safety semantics (durable state before the first delete, identity bound to a physical hub, refusal behaviour), not the vocabulary. `validate` is refused for the Collection decommission role (see #284 for the collection-wide audit) |
