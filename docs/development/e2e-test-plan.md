...
| R2 | Passive + ArgoCD mgmt1→mgmt2 | `python acm_switchover.py --primary-context mgmt1 --secondary-context mgmt2 --method passive --old-hub-action secondary --manage-auto-import-strategy --min-managed-clusters 3 --argocd-manage --disable-observability-on-secondary --verbose` | ArgoCD apps paused before switchover, advisory to resume after Git retarget | ✅ PASS | ~13 min, 10 apps paused; resume via --argocd-resume-only |
