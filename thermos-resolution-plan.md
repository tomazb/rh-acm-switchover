...
| 23 | ready_for_review | `fix/thermos-23-argocd-dry-run-parity` | `.worktrees/thermos-23-argocd-dry-run` | F40 | https://github.com/tomazb/rh-acm-switchover/pull/98 | `python -m pytest tests/test_primary_prep.py tests/test_main.py tests/test_main_phase_flow.py tests/test_argocd_coordinator.py tests/test_documentation_guardrails.py -q` passed; `python -m pytest ansible_collections/tomazb/acm_switchover/tests/unit/test_argocd_manage_role_contracts.py -q` passed; `python -m pytest ansible_collections/tomazb/acm_switchover/tests/integration/test_argocd_manage_role.py -q` passed; `git diff --check` passed; `graphify update .` passed; CodeRabbit CLI review `findings=0`. PR #98 opened; waiting after PR23 slice. |
