...
| F31 | confirmed hardening | PR 18 | Safe path validation/report artifact sanitization remains duplicated across Python and collection helpers, with different path bases and incomplete adversarial parity coverage. |
...
| F34 | confirmed robustness | PR 17 | Collection klusterlet remediation deletes `bootstrap-hub-kubeconfig` before recreating it, leaving a failure window with no bootstrap secret. |
...
| 13 | planned | `docs/thermos-round6-tracking` | `.worktrees/thermos-13-round6-tracking` | Round 6 tracker + PR 12 status drift + F30 verification | _TBD_ | Add F29-F36 tracking, mark PR 12 merged, and verify documentation guardrails plus CI-scope `black --check`. |
...
**Scope**
- Fix F31.
- Collapse duplicated safe-path/report-artifact validation logic into one canonical Python implementation plus a thin collection copy where import boundaries require it.
