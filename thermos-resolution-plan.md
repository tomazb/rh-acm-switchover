...
- `F44` Resolved by `PR 27`-`PR 31`: runtime/bootstrap, operation/phase-flow
  runners, Argo CD resume safety, and CLI outcome/report orchestration were
  extracted through the slice map established by the docs-only `PR 28`.
...
runtime/bootstrap seam, `PR 28` recorded the remaining slice map, and `PR 29`-
`PR 31` completed every mapped implementation seam.
...
`H1`, `H2`, and `M4` are now complete through PR #148, `PR 34` / PR #127, and
...
- `F44` is complete through `PR 27`-`PR 31`: PR #102 extracted runtime/bootstrap,
  PR #103 recorded the remaining slice map, PR #104 extracted operation and
  phase-flow runners, PR #106 extracted Argo CD resume safety, and PR #107
  extracted CLI outcome/report orchestration.

### Optional and deferred low-severity follow-ups
...
| F44 | resolved | PR 27-PR 31 | `PR 27` extracted runtime/bootstrap; docs-only `PR 28` fixed the remaining slice map; `PR 29`, `PR 30`, and `PR 31` completed operation/phase-flow runners, Argo CD resume safety, and CLI outcome/report orchestration respectively. GitHub PRs #102, #103, #104, #106, and #107 are merged, and the extracted `lib/` modules remain wired through `acm_switchover.py` with dedicated tests. |
